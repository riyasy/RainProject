...
RainDrop::RainDrop(const int windDirectionFactor, const RainDropType type, RainWindowData *windowData):
	Type(type), WindDirectionFactor(windDirectionFactor), pRainWindowData(windowData)
{
...
void RainDrop::InitializeMainDrop()
{
	...
	const int xWidenToAccountForSlant = RECT_WIDTH(pRainWindowData->WindowRect) / 3;
	Ellipse.point.x = static_cast<float>(RandomGenerator::GetInstance().GenerateInt(
		pRainWindowData->WindowRect.left - xWidenToAccountForSlant, pRainWindowData->WindowRect.right + xWidenToAccountForSlant));

	// Randomize y position
	const int y = (RandomGenerator::GetInstance().GenerateInt(-RECT_HEIGHT(pRainWindowData->WindowRect) / 2, pRainWindowData->WindowRect.top) / 10) * 10;
	Ellipse.point.y = static_cast<float>(y);

	// Create drop with radius ranging from 0.2 to 0.7 pixels
	Ellipse.radiusX = (RandomGenerator::GetInstance().GenerateInt(2, 7) / 10.0f) * pRainWindowData->ScaleFactor;
	Ellipse.radiusY = Ellipse.radiusX;

	// Initialize velocity and physics parameters
	DeltaX_PerPhysicsFrame = HORIZONTAL_SPEED_OF_DROP_BASE * PHYSICS_FRAME_INTERVAL * pRainWindowData->ScaleFactor * WindDirectionFactor;
	DeltaY_PerPhysicsFrame = VERTICAL_SPEED_OF_DROP * PHYSICS_FRAME_INTERVAL * pRainWindowData->ScaleFactor;
...
void RainDrop::InitializeSplatter()
{
	...
	Ellipse.radiusX = (RandomGenerator::GetInstance().GenerateInt(15, 25) / 10.0f) * pRainWindowData->ScaleFactor;
	Ellipse.radiusY = Ellipse.radiusX;
}
...
void RainDrop::MoveToNewPosition()
{
	...
	if (Type == RainDropType::Splatter)
	{
		...
		const float deltaYAdjustmentDueToGravity = GRAVITY * PHYSICS_FRAME_INTERVAL * pRainWindowData->ScaleFactor;
		DeltaY_PerPhysicsFrame += deltaYAdjustmentDueToGravity;
		const float deltaXAdjustmentDueToAirResistance = AIR_RESISTANCE * PHYSICS_FRAME_INTERVAL * pRainWindowData->ScaleFactor;
		...
		if (Ellipse.point.x + Ellipse.radiusX > pRainWindowData->WindowRect.right || Ellipse.point.x - Ellipse.radiusX < pRainWindowData->WindowRect.left)
		{
			DeltaX_PerPhysicsFrame = -DeltaX_PerPhysicsFrame;
		}

		// Check for bouncing against bottom border
		if (Ellipse.point.y + Ellipse.radiusY > pRainWindowData->WindowRect.bottom)
		{
			Ellipse.point.y = pRainWindowData->WindowRect.bottom - Ellipse.radiusY; // Keep the ellipse within bounds
			DeltaY_PerPhysicsFrame = -DeltaY_PerPhysicsFrame * BOUNCE_DAMPING; // Bounce with damping
			SplatterBounceCount++;
		}
		// Check for bouncing against top
		if (Ellipse.point.y - Ellipse.radiusY < pRainWindowData->WindowRect.top)
		{
	...
	if (Type == RainDropType::MainDrop)
	{
		if (!TouchedGround)
		{
			...
			{
				TouchedGround = true;
				Ellipse.point.y = static_cast<float>(pRainWindowData->WindowRect.bottom);

				if (MathUtil::IsPointInRect(pRainWindowData->WindowRect, Ellipse.point))
				{
...
void RainDrop::CreateSplatters()
{
	...
	{
		auto splatter = new RainDrop(0, RainDropType::Splatter, pRainWindowData);
		...
		xVelocityForSplatter *= pRainWindowData->ScaleFactor;
		yVelocityForSplatter *= pRainWindowData->ScaleFactor;
...
void RainDrop::Draw(ID2D1DeviceContext* dc) const
{
	if (Type == RainDropType::MainDrop && !TouchedGround)
	{
		...
		if (MathUtil::IsPointInRect(pRainWindowData->WindowRect, Ellipse.point) && MathUtil::IsPointInRect(pRainWindowData->WindowRect, prevPoint))
		{
			dc->DrawLine(prevPoint, Ellipse.point, pRainWindowData->DropColorBrush.Get(), Ellipse.radiusX);
		}
		else if (MathUtil::IsPointInRect(pRainWindowData->WindowRect, Ellipse.point) || MathUtil::IsPointInRect(pRainWindowData->WindowRect, prevPoint))
		{
			D2D1_POINT_2F adjustedPoint1, adjustedPoint2;
			MathUtil::TrimLineSegment(pRainWindowData->WindowRect, prevPoint, Ellipse.point, adjustedPoint1, adjustedPoint2);
			dc->DrawLine(adjustedPoint1, adjustedPoint2, pRainWindowData->DropColorBrush.Get(), Ellipse.radiusX);
		}
	}
	if (!Splatters.empty())
	{
		for (const auto drop : Splatters)
		{
			drop->DrawSplatter(dc, pRainWindowData->PrebuiltSplatterOpacityBrushes[CurrentFrameCountForSplatter].Get());
		}
	}
}
...
{
	if (Type == RainDropType::Splatter &&
		MathUtil::IsPointInRect(pRainWindowData->WindowRect, Ellipse.point) &&
